...
function createOrg3 {
  ...
  set -x
  fabric-ca-client register --caname ca-org3 --id.name partner --id.secret partnerpw --id.type client --tls.certfiles "${PWD}/fabric-ca/org3/tls-cert.pem"
  { set +x; } 2>/dev/null
  ...
  set -x
	fabric-ca-client enroll -u https://partner:partnerpw@localhost:11054 --caname ca-org3 -M "${PWD}/../organizations/peerOrganizations/org3.example.com/users/partner@org3.example.com/msp" --tls.certfiles "${PWD}/fabric-ca/org3/tls-cert.pem"
  { set +x; } 2>/dev/null

  cp "${PWD}/../organizations/peerOrganizations/org3.example.com/msp/config.yaml" "${PWD}/../organizations/peerOrganizations/org3.example.com/users/partner@org3.example.com/msp/config.yaml"
