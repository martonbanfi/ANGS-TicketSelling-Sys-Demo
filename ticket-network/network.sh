...
function clearContainers() {
  ...
  ${CONTAINER_CLI} rm -f $(${CONTAINER_CLI} ps -aq --filter name='ticket-system-user-login') 2>/dev/null || true
}
...
function networkUp() {
  ...
  ${CONTAINER_CLI} run --name ticket-system-user-login -p 3306:3306 -e MYSQL_ROOT_PASSWORD=ticketsyspassword -d mysql:oracle
...
function networkDown() {
  ...
  #remove wallet folder under ticket-system-router if exists
  infoln "Removing wallet folder under ticket-system-router"
  rm -r ${PWD}/../ticket-system-router/wallet
